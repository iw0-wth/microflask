...
#Date : 2026.3.21
#Description : MicroFlask is a Flask-like web framework engineered to streamline the migration of Flask-based projects.
...
"""
MicroFlask Web Framework
同步版本的MicroFlask，实现基本兼容Flask,可在mpy,py双环境运行，支持自定义动态路由规则
...
def _is_generator(obj):
    # 兼容 MicroPython：用“生成器类型”判断（借鉴 easyweb.py）
    def _g():
        yield 0
...
def _to_bytes(x, charset="utf-8"):
    ...
    # 允许 generator yield dict 作为 header patch
...
class Response:
    ...
    def __init__(self, content=b"", status=200, headers=None, mimetype=None, content_type=None):
        ...
        self._cookies = []  # list[str] like: Set-Cookie: ...
    ...
    def build(self):
        """仅用于非流式快速拼接（流式请用 iter_chunks）。"""
        if _is_streaming_body(self.content):
            # 避免把 generator 全部拼进内存
        ...
        body_bytes = _to_bytes(body)
        self._finalize_headers(body_bytes=body_bytes, streaming=False)
        return self._status_line() + self._headers_bytes() + body_bytes
...
class Application:
    ...
    def route(self, path, methods=None):
        if methods is None:
            methods = ["GET"]
        # 这里接收三个返回值
        compiled_pattern, converters, param_names = self._compile_route(path)

        def decorator(func):
            for method in methods:
                m = (method or "GET").upper()
                if "<" in path:
                    # 将 converters 和 param_names 一起存储
                    self.routes.append((m, compiled_pattern, converters, param_names, func))
                else:
    ...
    def add_route(self, path, handler, methods=None):
        ...
        for method in methods:
            ...
            if "<" in path:
                # 将 converters 和 param_names 一起存储
                self.routes.append((m, compiled_pattern, converters, param_names, handler))
            else:
                self.static_routes[(m, path)] = handler

    def run(self, host="0.0.0.0", port=80, debug=False, **kwargs):
        server = HTTPServer(self, host=host, port=port, **kwargs)
        server.start()
...
class HTTPServer:
    def __init__(self, app, host="0.0.0.0", port=80, max_request_length=8192, header_chunk_size=256):
        self.app = app
        self.host = host
        self.port = int(port)
        self.max_request_length = int(max_request_length)
        self.header_chunk_size = int(header_chunk_size)
    ...
    def start(self):
        ...
        self.sock.listen(5)
        print("[web]Server running on {}:{}".format(self.host, self.port))
        ...
        while True:
            ...
            _schedule(self.handle_connection, conn)

    def _send_all(self, conn, data):
        # MicroPython socket 可能没有 sendall
        mv = memoryview(data)
        total = len(mv)
        sent = 0
        while sent < total:
    ...
    def _read_until(self, conn, marker=b"\r\n\r\n"):
        buf = b""
        while True:
            chunk = conn.recv(self.header_chunk_size)
            if not chunk:
                break
            buf += chunk
            if marker in buf:
                return buf
            if len(buf) > self.max_request_length:
                raise Exception("Request too large")
        return buf

    def _parse_header_bytes(self, header_bytes):
        # header_bytes: bytes up to \r\n\r\n (may include some body tail)
        header_end = header_bytes.find(b"\r\n\r\n")
        if header_end < 0:
            raise Exception("Bad request")
        head = header_bytes[:header_end].decode("utf-8")
        rest = header_bytes[header_end + 4 :]
        lines = head.split("\r\n")
        if not lines:
            raise Exception("Bad request")
        ...
        method, full_path, protocol = req_line[0], req_line[1], req_line[2]
        headers = {}
        for line in lines[1:]:
            if not line:
                continue
        ...
        return method.upper(), full_path, protocol, headers, rest

    def handle_connection(self, conn):
        try:
            head_and_maybe_body = self._read_until(conn)
            method, full_path, protocol, headers, body0 = self._parse_header_bytes(
                head_and_maybe_body
            )
            content_length = 0
            try:
            ...
            body = body0 or b""
            remain = content_length - len(body)
            while remain > 0:
                chunk = conn.recv(min(1024, remain))
                if not chunk:
                    break
                body += chunk
                remain -= len(chunk)
            ...
            for chunk in resp.iter_chunks():
                if chunk:
                    self._send_all(conn, chunk)
        except Exception as e:
            print("[web]Error:", e)
            try:
                resp = Response("Internal Server Error", status=500)
                self._send_all(conn, resp.build())
            except Exception:
                pass
        finally:
            try:
